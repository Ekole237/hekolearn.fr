...
import { CreateChapterInput, CreateCourseInput, CreateLessonInput } from './types';
...
export async function publishCourse(courseId: string) {
  ...
  if (!course.chapters || course.chapters.length === 0) {
    throw new Error('Le cours doit avoir au moins un chapitre avant d\'être publié');
  }

  const hasLessons = course.chapters.some(chapter => 
    chapter.lessons && chapter.lessons.length > 0
  );

  if (!hasLessons) {
    throw new Error('Le cours doit avoir au moins une leçon avant d\'être publié');
  }
...
export async function updateChaptersOrder(chapters: { id: string; position: number }[]) {
  const supabase = createClientComponentClient<Database>();
...
export async function updateLessonsOrder(lessons: { id: string; position: number }[]) {
  const supabase = createClientComponentClient<Database>();
